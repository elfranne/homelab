...
| 2 | [Containers](<2 - Containers/INSTALL.md>) | Services on Incus: a shared [Caddy](https://caddyserver.com/) reverse-proxy ingress with real Let's Encrypt certificates (Cloudflare DNS-01, no open ports), built as a reproducible [distrobuilder](https://github.com/lxc/distrobuilder) image and updated by swapping its rootfs; and [Nextcloud](https://nextcloud.com/) as the first service (nginx + PHP-FPM + PostgreSQL + Redis), a stateful container upgraded in place, with data on its own encrypted ZFS volume. | Done |
